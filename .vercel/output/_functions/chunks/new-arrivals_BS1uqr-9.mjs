...
const $$NewArrivals = createComponent(async ($$result, $$props, $$slots) => {
  let products = [];
  let categories = [];
  try {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, {}, { "default": async ($$result2) => renderTemplate`  ${renderComponent($$result2, "Header", $$Header, {})} ${maybeRenderHead()}<main class="container mx-auto px-4 py-16 min-h-[60vh] max-w-lg md:max-w-6xl"> <div class="text-center mb-16"> <h1 class="text-4xl md:text-5xl font-bold text-[#2D2A26] mb-4 uppercase tracking-wider">New Arrivals</h1> <p class="text-[#A39E93] text-lg max-w-2xl mx-auto">Discover our latest collection of premium mural wall art and beautifully curated frames. Check back soon for exciting new additions.</p> </div> <!-- Frames Section --> <div class="mb-20"> <div class="mb-8"> <h2 class="text-3xl font-bold text-[#2D2A26] uppercase tracking-wider border-b-2 border-[#EBE6DC] pb-2 inline-block">Frames</h2> </div> ${processedProducts.length === 0 ? renderTemplate`<p class="text-center text-gray-500 text-xl py-12">No new frames currently available.</p>` : renderTemplate`<div class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 lg:gap-6"> ${processedProducts.map((product) => renderTemplate`<a${addAttribute(`/product/${product._id}`, "href")} class="block transition hover:-translate-y-1 h-full"> <div class="bg-[#FDFBF7] rounded-[24px] p-2 flex flex-col shadow-sm border border-[#F3EFE6] h-full group hover:shadow-md transition-shadow duration-300"> <div class="relative w-full aspect-square rounded-[20px] overflow-hidden mb-3 bg-[#F5F2EA]"> <img${addAttribute(product.imageUrl, "src")}${addAttribute(product.title, "alt")} class="w-full h-full object-cover mix-blend-multiply transition-transform duration-500 group-hover:scale-105"> </div> <div class="px-2 pb-2 flex flex-col flex-grow"> <h3 class="font-bold text-[#2D2A26] text-sm md:text-base leading-tight mb-1">${product.title}</h3> <div class="text-[#A39E93] text-xs mb-3 flex-grow flex flex-col gap-1"> <span class="truncate">Sizes: ${product.sizesPreview}</span> </div> <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-3 mt-auto"> <span class="text-[#C59F58] font-bold text-sm md:text-base">From ${formatNaira(product.minPrice)}</span> <div class="w-full md:hidden text-center bg-transparent border border-[#2D2A26] text-[#2D2A26] group-hover:bg-[#2D2A26] group-hover:text-white font-medium py-2 px-4 rounded transition-colors duration-300 uppercase tracking-wider text-xs">
View Product
</div> <div class="hidden md:flex w-8 h-8 shrink-0 rounded-full bg-[#F5F2EA] items-center justify-center text-[#C59F58] group-hover:bg-[#EAE5D9] transition"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <line x1="12" x2="12" y1="5" y2="19"></line> <line x1="5" x2="19" y1="12" y2="12"></line> </svg> </div> </div> </div> </div> </a>`)} </div>`} </div> <!-- Murals Section --> <div> <div class="mb-8"> <h2 class="text-3xl font-bold text-[#2D2A26] uppercase tracking-wider border-b-2 border-[#EBE6DC] pb-2 inline-block">Murals</h2> </div> ${categories.length === 0 ? renderTemplate`<p class="text-center text-gray-500 text-xl py-12">No new murals currently available.</p>` : renderTemplate`<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"> ${categories.map((category) => renderTemplate`<div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden flex flex-col group hover:shadow-md transition-shadow duration-300 h-full"> <div class="h-64 overflow-hidden relative"> <img${addAttribute(category.image, "src")}${addAttribute(category.title, "alt")} class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"> </div> <div class="p-6 flex flex-col flex-grow"> <h3 class="text-xl font-bold text-[#2D2A26] mb-3">${category.title}</h3> <p class="text-[#A39E93] mb-6 flex-grow">${category.description}</p> <a${addAttribute(`/catalogue/${category.slug}`, "href")} class="inline-block w-full text-center bg-transparent border border-[#2D2A26] text-[#2D2A26] hover:bg-[#2D2A26] hover:text-white font-medium py-3 px-6 rounded transition-colors duration-300 uppercase tracking-wider text-sm mt-auto">
View Catalogue
</a> </div> </div>`)} </div>`} </div> </main>  ${renderComponent($$result2, "Footer", $$Footer, {})} ` })}`;
}, "/app/src/pages/new-arrivals.astro", void 0);

const $$file = "/app/src/pages/new-arrivals.astro";
const $$url = "/new-arrivals";
